...
python3 script/dev-tools shell $EXTRA_OPTS --command "$COMMAND" \
    ...
    $EXE --config-path "conf/hstream.yaml" --port $SERVER_PORT --log-with-color --store-admin-port $LD_ADMIN_PORT --zkuri "127.0.0.1:$ZOOKEEPER_PORT" --server-id $SERVER_ID --address 127.0.0.1
