...
createSinkConnector :: T.Text -> IO Bool
createSinkConnector sql = withGRPCClient clientConfig $ \client -> do
  HStreamApi{..} <- hstreamApiClient client
  let createSinkConnectorRequest = CommandQuery { commandQueryStmtText = sql }
  resp <- hstreamApiExecuteQuery (ClientNormalRequest createSinkConnectorRequest 100 (MetadataMap Map.empty))
  case resp of
