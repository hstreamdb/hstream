...
module HStream.RegressionNewSpec (spec) where
...
import           HStream.SQL.Rts.New           (FlowValue (..),
                                                flowValueToJsonValue)
...
spec = aroundAll provideHstreamApi $
  describe "HStream.RegressionNewSpec" $ do
  ...
  it "#391_JOIN" $ \api -> do
    ...
    runCreateStreamSql api "CREATE STREAM s1 (a integer, b integer);"
    runCreateStreamSql api "CREATE STREAM s2 (a integer, b integer);"
    _ <- forkIO $ do
    ...
    runFetchSql "SELECT s1.b, SUM(s1.a), SUM(s2.a) FROM s1 INNER JOIN s2 ON s1.b = s2.b GROUP BY s1.b EMIT CHANGES;"
#else
    runFetchSql "SELECT s1.b, SUM(s1.a), SUM(s2.a) FROM s1 INNER JOIN s2 ON s1.b = s2.b WITHIN (INTERVAL 1 HOUR) GROUP BY s1.b EMIT CHANGES;"
#endif
  ...
  it "#403_RAW" $ \api -> do
    ...
    runCreateStreamSql api "CREATE STREAM s4(a INTEGER, b INTEGER);"
  ...
  xit "HS352_INT" $ \api -> do
    ...
    runCreateStreamSql api "CREATE STREAM s6 (key1 integer, key2 string, key3 boolean);"
  ...
  -- FIXME: what should it actually do in such cases?
  it "#1200_BINARY" $ \api -> do
    runDropSql api "DROP STREAM stream_binary IF EXISTS;"
    runCreateStreamSql api "CREATE STREAM stream_binary(value BYTEA);"
    _ <- forkIO $ do
      threadDelay 10000000 -- FIXME: requires a notification mechanism to ensure that the task starts successfully before inserting data
