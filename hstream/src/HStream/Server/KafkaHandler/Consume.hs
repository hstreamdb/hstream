...
module HStream.Server.KafkaHandler.Consume where
...
handleFetchV0 ServerContext{..} _ K.FetchRequestV0{..} = case topics of
  ...
  Just topicReqs_ -> do
              ...
              , acc_resps
               ...
               if acc_timeLeft < 0 then
                 ...
                 (totalMaxBytes_m', timeLeftMs', resp) <-
                   readSingleTopic topicReq acc_totalMaxBytes_m acc_timeLeft acc_isFirstTopic
    ...
    return $ K.FetchResponseV0 (Just $ V.fromList resps)
  where readSingleTopic :: K.FetchTopicV0
        ...
        readSingleTopic K.FetchTopicV0{..} totalMaxBytes_m timeLeftMs isFirstTopic = case partitions of
          Nothing    -> return (totalMaxBytes_m, timeLeftMs, K.FetchableTopicResponseV0 topic Nothing)
          Just parts -> do
            orderedParts <- S.listStreamPartitionsOrdered scLDClient (transToStreamName topic)
            -- FIXME: is it proper to use one reader for all partitions of a topic?
            reader <- S.newLDReader scLDClient 1 (Just 1)
              ...
              foldM (\( acc_isFirstPartition
                      ...
                      , acc_resps
                       ...
                       if acc_timeLeft <= 0 then
                         ...
                         (len,timeLeftMs',resp) <-
                           readSinglePartition reader logId partition fetchOffset acc_totalMaxBytes_m partitionMaxBytes acc_timeLeft acc_isFirstPartition isFirstTopic
            ...
            return ( totalMaxBytes_m'
                   , timeLeftMs'
                   , K.FetchableTopicResponseV0 topic (Just $ V.fromList resps)
                   )

        readSinglePartition :: S.LDReader   -- the logdevice reader of this **topic**, but only one logId is read at the same time
        ...
        readSinglePartition reader logId partitionIndex offset totalMaxBytes_m partitionMaxBytes timeLeftMs isFirstPartition isFirstTopic = do
          (_, startLSN) <- S.findKey scLDClient logId (U.int2cbytes offset) S.FindKeyStrict
          endLSN <- S.getTailLSN scLDClient logId
                  ...
                  case L.null records of
                    ...
                    False -> do
                      let record = L.head records
                          recordBytes = S.recordPayload record
                          recordBytesLen = BS.length recordBytes
