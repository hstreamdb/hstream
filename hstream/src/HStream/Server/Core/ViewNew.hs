...
module HStream.Server.Core.ViewNew where
...
executeViewQueryWithNamespace sc@ServerContext{..} sql namespace = parseAndBind sql (P.getSchema metaHandle) >>= \case
  BoundQSelect select_imm -> do
    ...
    (BoundQCreate (BoundCreateView _ select_v))
    ...
    relationExpr_v <- planIO select_v (P.getSchema metaHandle)
    -- 3.1 scan for HAVING
    let trans_m_having =
          case scanRelationExpr (\r -> case r of
            ...
            (Just (Filter schema r' hav_scalar'), _) -> -- can not determine WHERE or HAVING
              case scanRelationExpr (\r -> case r of
                                        Reduce{} -> True
                                        _        -> False
                                    ) Nothing r' of
                (Nothing, _) -> Nothing -- only one Filter but no Reduce, it is WHERE
                (Just _ , _) -> Just (\r -> Filter schema r hav_scalar') -- both Filter and Reduce, it is HAVING
    ...
    let trans_m_project =
          case scanRelationExpr (\r -> case r of
            ...
            (Nothing                        , _) -> Nothing
            (Just (Project schema _ tups), _) -> Just (\r -> Project schema r tups)
