...
appendBatchBS client logid payloads compression m_key_attr =
  let exbs (BS.PS payload off len) = (payload, fromIntegral off, fromIntegral len)
      (ps, offs, lens) = unzip3 (map exbs payloads)
   in withForeignPtr client $ \client' ->
      Z.withPrimArraySafe (Z.primArrayFromList lens) $ \lens' num ->
      Z.withPrimArraySafe (Z.primArrayFromList offs) $ \offs' _num_offs ->
...
foreign import ccall unsafe "hs_logdevice.h logdevice_append_batch_rts"
  logdevice_append_batch_rts
    :: Ptr LogDeviceClient
    -> C_LogID
