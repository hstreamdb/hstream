...
#define APPEND_BATCH(ExPayload, ExOffset, ExLen, total_len)                    \
  do {                                                                         \
    BufferedWriteCodec::Estimator blob_size_estimator;                         \
    for (int i = 0; i < total_len; ++i) {                                      \
      auto iobuf =                                                             \
          folly::IOBuf::wrapBufferAsValue((char*)ExPayload + ExOffset, ExLen); \
    ...
    int rv =                                                                   \
        client_impl->appendBatched(logid_t(logid), std::move(payload_holder),  \
...
facebook::logdevice::Status logdevice_append_batch_rts(
    logdevice_client_t* client, c_logid_t logid,
    // Payloads
  ...
  // NOTE: this will implicit convert HsInt to int
  APPEND_BATCH(payloads[i]->payload, 0, payload_lens[i], total_len);
}
