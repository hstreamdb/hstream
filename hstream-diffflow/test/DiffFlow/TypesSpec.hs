...
--------------------------------------------------------------------------------
      ...
      , dcbChanges = [ DataChange (A.fromList [("a", Number 1)]) (Timestamp 0 []) 1
                     , DataChange (A.fromList [("b", Number 2)]) (Timestamp 0 []) 1
...
index = describe "Index" $ do
  it "add DataChangeBatch to Index" $ do
    ...
    addChangeBatchToIndexChecker
      ...
      (Index [ mkDataChangeBatch [ DataChange (A.fromList [("g", Null)]) (Timestamp (0 :: Int) []) 1 ]
             , mkDataChangeBatch [ DataChange (A.fromList [("e", Null)]) (Timestamp (0 :: Int) []) 1
